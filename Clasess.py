...
class RespondGet(ExcelReader):
    ...
    def resp_get_null(self):
        try:
          ...
          ex = df.isna().sum().sum()
          ex_str = str(ex)
          return jsonify({"value" : str(ex_str)}), 200
    ...
    def resp_get_ex_val(self):
        try:
          ...
          return jsonify({'value' : str(dat[2]), 
                        'columns' : str(dat[1]), 
                         "row" : str(dat[0])}), 200
        except Exception as e:
          return jsonify({"error": f"An error occurred: {str(e)}"}), 500
    ...
    def resp_get_ex_sumar(self):
         try:
          ...
          return jsonify({"col_dtypes": col_dtypes,
                          "rows": rows,
          "columns": columns,
    ...
    def chart_columns(self):
      try:
        ...
        return jsonify({"value" : columns}), 200
      except Exception as e:
        return jsonify({"error": f"An error occurred: {str(e)}"}), 500
       ...
       try:   
        ...
        for column in df:
          for index, val in df[column].items():
              val, modify = response_fill.highlight_null(val)
          ...
          for column, val, modify in data:  
           result[column]['arr'].append({'value': val, 'status': modify})
        return result, 200
       except Exception as e:
        return jsonify({"error": f"An error occurred: {str(e)}"}), 500
...
class Chart( ExcelReader):
  ...
  def generare_grafic(self, data):
     try:
      ...
      return send_file('grafic.png', mimetype='image/png'), 200
     except Exception as e:
      return jsonify({"error": f"An error occurred: {str(e)}"}), 500
